...
def gen_signal(B=20, fs=25000, varsize=25000, fmod=30, f_center=5000, bandwidth=1500, shift=0, sigma = 1):
    ...
    noise = np.random.normal(loc=0,scale=sigma,size=varsize)
...
if __name__== "__main__":
    B = int(input("amplituda impulsów: "))      
    fs = int(input("częstotliwość próbkowania: "))          
    varsize = int(input("długość sygnału: "))        

    # Parametry impulsów
    fmod = int(input("częstotliwość powtarzania impulsów: "))
    f_center = int(input("środek pasma: "))     
    bandwidth = int(input("szerokość pasma: "))
    shift = int(input("przesunięcie impulsów: "))
    sigma = int(input("odchylenie szumu: "))
    np.savetxt("assets/signal.csv",gen_signal(B, fs, varsize, fmod, f_center, bandwidth, shift, sigma),delimiter=",")
